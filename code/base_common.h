...
typedef int (*FunctionCallbackV)(void*);
...
// 打印函数
int function_print_(char* name, void* callback, void* args) {
    int ret = 0;
    if (name == NULL) {
        name = "default";
    }
    ...
    if (args == NULL) {
        ...
        FunctionCallbackV func = (FunctionCallbackV)callback;
        pArgs p_arg = (pArgs) args;
        if (p_arg->intset) {
            ret = func(&p_arg->argint);
        } else if (p_arg->charset) {
            ret = func(&p_arg->argchar);
        }
    }
    printf("[%s test] ret: %d\n", name, ret);
...
int function_print(char* name, void* callback) {
    return function_print_(name, callback, NULL);
}